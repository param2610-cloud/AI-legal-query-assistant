...
# Path to Drug and Cosmetics Act PDF
pdf_path = "drug_cosmeticsa1940-23.pdf"

print("📖 Starting Drug and Cosmetics Act PDF extraction...")

# Initialize data structure for JSON storage
drug_cosmetics_act_data = {
    "metadata": {
        "act_name": "Drug and Cosmetics Act, 1940",
        "extraction_date": datetime.now().isoformat(),
        "source_file": "drug_cosmeticsa1940-23.pdf",
        "total_sections": 0
    },
    "sections": []
}
...
try:
    ...
    for section_id, section_info in sections.items():
        ...
        drug_cosmetics_act_data["sections"].append(section_data)

    # Update metadata with total sections count
    drug_cosmetics_act_data["metadata"]["total_sections"] = len(drug_cosmetics_act_data["sections"])

    # Output as JSON (original format)
    with open("drug_cosmetics_sections.json", "w", encoding='utf-8') as f:
        json.dump(sections, f, indent=2, ensure_ascii=False)

    # Output structured format (similar to consumer act)
    with open("drug_cosmetics_act_structured.json", "w", encoding='utf-8') as f:
        json.dump(drug_cosmetics_act_data, f, indent=2, ensure_ascii=False)
    ...
    sections_only = [
        ...
        for section in drug_cosmetics_act_data["sections"]
    ]

    with open("drug_cosmetics_act_sections_only.json", "w", encoding='utf-8') as f:
        json.dump(sections_only, f, indent=2, ensure_ascii=False)

    print("✅ Drug and Cosmetics Act sections saved to drug_cosmetics_sections.json")
    print("✅ Structured format saved to drug_cosmetics_act_structured.json")
    print("✅ Simplified sections saved to drug_cosmetics_act_sections_only.json")
    print(f"📊 Total sections extracted: {drug_cosmetics_act_data['metadata']['total_sections']}")
    ...
    print("Also ensure that drug_cosmeticsa1940-23.pdf exists in the current directory")