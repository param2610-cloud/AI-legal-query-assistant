...
# Path to Aadhaar Act PDF
pdf_path = "adhaar_act.pdf"

print("📖 Starting Aadhaar Act PDF extraction...")

# Initialize data structure for JSON storage
aadhaar_act_data = {
    "metadata": {
        "act_name": "Aadhaar (Targeted Delivery of Financial and Other Subsidies, Benefits and Services) Act",
        "extraction_date": datetime.now().isoformat(),
        "source_file": "adhaar_act.pdf",
        "total_sections": 0
    },
    "sections": []
}
...
try:
    ...
    if temp_sections and len(temp_sections) > 10:
    ...
    for section_id, section_info in sections.items():
        ...
        aadhaar_act_data["sections"].append(section_data)

    # Update metadata with total sections count
    aadhaar_act_data["metadata"]["total_sections"] = len(aadhaar_act_data["sections"])

    # Output as JSON (original format)
    with open("aadhaar_sections.json", "w", encoding='utf-8') as f:
        json.dump(sections, f, indent=2, ensure_ascii=False)

    # Output structured format (similar to consumer act)
    with open("aadhaar_act_structured.json", "w", encoding='utf-8') as f:
        json.dump(aadhaar_act_data, f, indent=2, ensure_ascii=False)
    ...
    sections_only = [
        ...
        for section in aadhaar_act_data["sections"]
    ]

    with open("aadhaar_act_sections_only.json", "w", encoding='utf-8') as f:
        json.dump(sections_only, f, indent=2, ensure_ascii=False)

    print("✅ Aadhaar Act sections saved to aadhaar_sections.json")
    print("✅ Structured format saved to aadhaar_act_structured.json")
    print("✅ Simplified sections saved to aadhaar_act_sections_only.json")
    print(f"📊 Total sections extracted: {aadhaar_act_data['metadata']['total_sections']}")
