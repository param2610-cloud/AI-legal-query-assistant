...
# Path to Civil Procedure Act PDF
pdf_path = "civil_procedure_act.pdf"

print("📖 Starting Civil Procedure Act PDF extraction...")

# Initialize data structure for JSON storage
civil_procedure_act_data = {
    "metadata": {
        "act_name": "Civil Procedure Act",
        "extraction_date": datetime.now().isoformat(),
        "source_file": "civil_procedure_act.pdf",
        "total_sections": 0
    },
    "sections": []
}
...
try:
    ...
    for section_id, section_info in sections.items():
        ...
        civil_procedure_act_data["sections"].append(section_data)

    # Update metadata with total sections count
    civil_procedure_act_data["metadata"]["total_sections"] = len(civil_procedure_act_data["sections"])

    # Output as JSON (original format)
    with open("civil_procedure_sections.json", "w", encoding='utf-8') as f:
        json.dump(sections, f, indent=2, ensure_ascii=False)

    # Output structured format (similar to consumer act)
    with open("civil_procedure_act_structured.json", "w", encoding='utf-8') as f:
        json.dump(civil_procedure_act_data, f, indent=2, ensure_ascii=False)
    ...
    sections_only = [
        ...
        for section in civil_procedure_act_data["sections"]
    ]

    with open("civil_procedure_act_sections_only.json", "w", encoding='utf-8') as f:
        json.dump(sections_only, f, indent=2, ensure_ascii=False)

    print("✅ Civil Procedure Act sections saved to civil_procedure_sections.json")
    print("✅ Structured format saved to civil_procedure_act_structured.json")
    print("✅ Simplified sections saved to civil_procedure_act_sections_only.json")
    print(f"📊 Total sections extracted: {civil_procedure_act_data['metadata']['total_sections']}")
    ...
    print("Also ensure that civil_procedure_act.pdf exists in the current directory")